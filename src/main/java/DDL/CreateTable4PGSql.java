 ...
 **/
public class CreateTable4PGSql {
    ...
    public static String printSql(List<List> tables){
        ...
        for(List table:tables){
            ...
            for (int i = 0;i<columns.size();i++){
                if (i == columns.size()-1){
                    createSql+= String.format("%-"+ String.valueOf((Integer)lens.get(0)+4)+"s",columns.get(i).get(0)) + String.format("%-"+ String.valueOf((Integer) lens.get(1)+4)+"s",columns.get(i).get(1)) +"\n";
                }else {
                    createSql+= String.format("%-"+ String.valueOf((Integer)lens.get(0)+4)+"s",columns.get(i).get(0)) + String.format("%-"+ String.valueOf((Integer)lens.get(1)+4)+"s",columns.get(i).get(1)) + ",\n";
                }
            }
