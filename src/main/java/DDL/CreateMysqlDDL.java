...
//        String filePathxlsx = "C:\\Users\\Qiang\\Desktop\\余杭大数据\\01_ODS\\ODS_数据字典.xlsx";
    ...
    public static String printSql(List<List> tables){
        ...
        for(List table:tables){
            ...
            for (int i = 0;i<columns.size();i++){
                if (i == columns.size()-1){
                    createSql+= String.format("%-"+ String.valueOf((int)lens.get(0)+4)+"s",columns.get(i).get(0)) + String.format("%-"+ String.valueOf((int)lens.get(1)+4)+"s",columns.get(i).get(1)) + "comment '"+ columns.get(i).get(2) + "'\n";
                }else {
                    createSql+= String.format("%-"+ String.valueOf((int)lens.get(0)+4)+"s",columns.get(i).get(0)) + String.format("%-"+ String.valueOf((int)lens.get(1)+4)+"s",columns.get(i).get(1)) + "comment '"+ columns.get(i).get(2) + "',\n";
                }
            }
