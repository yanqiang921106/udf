...
public class cube4FourDimPro extends UDTF {
...
//        指标数组
        ...
        for(int i=1;i<=dim_cnt;i++){
            dim_list.add(args[index_cnt+i].toString().concat(splitStr).concat(default_str));
        }
        ...
        if(!dim_list.isEmpty() && dim_list.size()>=2){
            ...
            dim_list.remove(0);
            combination = myutils.cycle(dim_first,dim_list,splitStr);
...
//                forward(out[0],out[1],out[2],out[3],out[4]);
                forward(out);
            }
        }else {
            for(String res:combination) {
