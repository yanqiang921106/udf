...
@Resolve({"string,bigint,double,double,bigint,bigint,bigint,bigint,double,double,string,string,string,bigint,string" +
        "->" +
        "string,string,string,string,string,double,double,bigint,bigint,bigint,bigint,double,double"})
public class cube4StkIndex extends UDTF {
...
//        if(!default_str.toLowerCase().equals("99999")){
//            throw new UDFException("default param is valid!");
//        }

        if(default_str == null || default_str.length() == 0){
            throw new UDFException("default param is valid!");
        }
...
//        指标数组
        ...
        if(index.length>0){
            for(String res:combination) {
                ...
                forward(out[0],out[1],out[2],out[3],out[4],Double.valueOf(out[5]),Double.valueOf(out[6]),Long.valueOf(out[7]),
                        Long.valueOf(out[8]),Long.valueOf(out[9]),Long.valueOf(out[10]),Double.valueOf(out[11]),Double.valueOf(out[12]));
            }
        }else {
            for(String res:combination) {
                out = res.split(":::");
